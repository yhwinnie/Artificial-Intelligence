...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        ghost = None
        best_action = None
        # Ghosts that are uncaptured 
        for pos in livingGhostPositionDistributions:
            ghost = pos.argMax()
        
            for action in legal:
                # where pacman will be after taking action 
                successorPosition = Actions.getSuccessor(pacmanPosition, action)
                # maze distance between pacman's new position and the ghost
                distance = self.distancer.getDistance(successorPosition, ghost)
                # want to find the smallest distance 
                if distance < dist:
                    dist = distance
                    best_action = action
        return best_action
